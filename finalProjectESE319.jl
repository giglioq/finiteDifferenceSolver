...
md"""
[Maki-e]("https://makie.juliaplots.org/stable/")
...
# ╔═╡ 50ed67bb-b218-4ec4-9707-153c4adc9325
delta = .1 #mesh size

# ╔═╡ 2f8cdbc6-94f4-435f-83dc-a7bb77bb4a62
dx = dy = delta

# ╔═╡ f8f5c711-400f-4a5d-8495-e78a7f295327
x = y = 0:delta:L
#This is the entire mesh, the boundary conditions are applied later

# ╔═╡ 7af832a7-afb8-47f3-83f6-9cf6860b1d43
sl = dx*dy/(c*sqrt(dx^2 + dy^2)) 
#stability limit, dt < sl or the scheme is unstable
...
#initial coniditions, v(x,y,t) = ∂u/∂t; v(x,y,0) = 0;

# ╔═╡ 08cc214e-93fe-4fcf-ac91-49479fd4c6a8
f(n::Int) = zeros(length(x), length(y)) #optional forcing terms;

# ╔═╡ 48c88431-4f97-4c59-83f7-eac115f6a1b1
ux0 = uxL = zeros(length(x))# x Boundary conditions, u(0,y,t) = u(L,y,t) = 0;
	...
	for n in 2:length(t)-1 
		...
		for i in 2:length(x)-1
			for j in 2:length(y)-1
				...
				u_yy = u[i, j-1, n] - 2*u[i, j, n] + u[i, j+1, n]
				u[i,j,n+1] = 2 * u[i,j, n] - u[i,j, n-1] + cx2 * u_xx + cy2 * u_yy 
				#+ dt2*f(n)[i,j]
			end
		end
		# insist boundaries stay clamped
